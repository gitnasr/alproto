 ...
 *      next/link instead, so a link stays a link for the keyboard, the context
 *      menu, and the crawler.
 *   3. The rAF loop pauses while the button is off-screen. Each instance owns a
 *      WebGL context and a render loop, so a page with several of them would
 *      otherwise redraw all of them every frame, forever.
 */
...
const SpecularButton = ({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // and it is discarded with the effect.
  ...
  return (
    <button type={type} disabled={disabled} {...shared} {...rest}>
      {inner}
    </button>
  );
};
