...
/**
 * OptionWheel — reactbits.dev (JavaScript + CSS variant), vendored verbatim
 * apart from the "use client" directive and the local CSS import path.
 *
 * No dependencies and no canvas: it lays the options out along a circle by
 * writing transform/opacity/filter from a single rAF loop that parks itself
...
const OptionWheel = ({
  ...
  useEffect(() => {
    ...
    /* SILOSAGE: upstream always preventDefaults the wheel event, which takes
       ...
       wheel is still driven by drag, click and the arrow keys. */
    if (!el || !captureScroll) return;
    const onWheel = (e) => {
      e.preventDefault();
      const cfg = cfgRef.current;
