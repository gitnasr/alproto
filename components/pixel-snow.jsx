...
/**
 * PixelSnow — reactbits.dev (JavaScript + CSS variant), vendored verbatim
 * apart from the "use client" directive and the local CSS import path.
 *
 * Worth knowing before touching the props: the fragment shader raymarches up
 * to 128 steps per pixel, so the cost scales with how many pixels it actually
 * draws. `pixelResolution` is the dial that matters — it renders chunky pixels
 * and is what keeps a full-viewport instance affordable.
 */
  ...
  for (int i = 0; i < 128; i++) {
    ...
    if (cellHash < uDensity) {
      ...
      if (toIntersection > 0.0) {
        ...
        // exactly that many pixels at any depth. Brightness still falls off
        // with distance, through uFlakeSize / flakeSize and uDepthFade.
        float flakeSize = max(uFlakeSize, uMinFlakeSize * depth * halfInvResX);
        flakeSize = min(flakeSize, uMaxFlakeSize * depth * halfInvResX);
...
export default function PixelSnow({
  ...
  useEffect(() => {
    ...
    material.uniforms.uMaxFlakeSize.value = maxFlakeSize;
